...
class COCO(PairedDataset):
    ...
    def get_samples(self, roots, ids_dataset=None):
        ...
        for split in ['train', 'val', 'test']:
            ...
            # file = open('imageIndexes.txt','a' , encoding='utf-8')
            # # if ids_dataset is None:
            # file.write(str([list(ids_dataset['train'][0]) ,list(ids_dataset['train'][1])])+"\n")
            # file.write(str([list(ids_dataset['trainrestval'][0]) , list(ids_dataset['trainrestval'][0])])+"\n")
            # file.write(str(list(ids_dataset['test']))+"\n")
            # file.write(str(list(ids_dataset['val']))+"\n")
            # file.close()
            # dictIndex = dict()
            # dictIndex['train'] = list(ids_dataset['train'])
            # dictIndex['trainrestval'] = list(ids_dataset['trainrestval'])
            # dictIndex['test'] = list(ids_dataset['test'])
            # dictIndex['val'] = list(ids_dataset['val'])
            # import json
            # with open('indexes.json', 'w') as fout:
            #     json.dump(dictIndex, fout)
